        ...
        amenities: {
          ...
          items: {
            ...
            properties: {
              ...
              stations: { type: 'array', items: { type: 'string' }, description: 'Explicit whitelist of real Warsaw metro station names to measure walking distance to (metro only). Resolve descriptive constraints into a concrete list, e.g. "M1 stations plus 1-2 stops from Świętokrzyska on M2" → the actual station names. Use exact names from the METRO REFERENCE in the system prompt; never invent a station. When set, this replaces `line`.' },
            },
            required: ['type', 'maxMinutes'],
          },
          description: 'Desired nearby amenities with maximum walking time and optional Warsaw metro line or station whitelist',
        },
        ...
        amenities: {
          ...
          items: {
            ...
            properties: {
              ...
              line: { type: 'string', enum: ['M1', 'M2'], description: 'Required Warsaw metro line. Only valid when type is metro.' },
              stations: { type: 'array', items: { type: 'string' }, description: 'Explicit whitelist of real Warsaw metro station names (metro only), resolved from a descriptive constraint. Exact names from the METRO REFERENCE in the system prompt; never invent. Replaces `line` when set.' },
            },
            required: ['type', 'maxMinutes'],
          },
